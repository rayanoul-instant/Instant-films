...
export default function DirectorPage() {
  ...
  const countLabel = `${count} court${count > 1 ? 's' : ''} métrage${count > 1 ? 's' : ''}`;

  usePageMeta(
    director ? `${director} — Courts métrages | Instant Films` : 'Instant Films',
    director ? `Découvrez les ${countLabel} de ${director} disponibles sur Instant Films.` : undefined
  );
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {!isLoading && !director ? (
          <p className="text-muted-foreground">Réalisateur introuvable.</p>
        ) : (
          <>
            <h1 className="font-display text-3xl font-bold mb-2">{director || '...'}</h1>
            <p className="text-muted-foreground mb-8">{countLabel} disponible{count > 1 ? 's' : ''}</p>
